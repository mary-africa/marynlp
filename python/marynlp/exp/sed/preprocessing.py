...
import re
import numpy as np
from sed.heuristic import MorphologyAnalyzer
...
class LabelEncoder:
    ...
    def encode(self, item):
        if item in self.items:
            return int(self.items.index(item))
...
class Tokenizer(MorphologyAnalyzer):
    ...
    def get_breaks(self, word, stems):
        ...
        for br in init_break:
            if br.rfind('ni')==0: 
                ...
                morphs += [br for br in interm_break if br not in '' and len(br)>2 or (len(br)==1 and br in self.vowels and (word.find(br)==0 or word.rfind(br)==len(word)-1))\
                    or (len(br)==2 and br[0] not in self.vowels)]
            else:
                interm_break = []
                morphs += [br for br in init_break if br.find(breaks[0])!=0 and br not in '' and len(br)>2 or (len(br)==1 and br in self.vowels and word.find(br)==0\
                    or word.rfind(br)==len(word)-1) or (len(br)==2 and br[0] not in self.vowels)]+interm_break
    ...
    def break_corpus(self, stems, corpus):
        break_dict = {}
    ...
    def fit(self, corpus = None, corpus_path = None, return_breaks=False, token_store_path=None):
        '''
        ...
        stems = sorted([k for k in self.morphemes.keys() if k != 'breaks'], key=len, reverse=True)

        break_dict = self.break_corpus(stems, corpus)

        if not isinstance(self.encoder, LabelEncoder): 
            self.initiate_encoder(break_dict, token_store_path)

        if return_breaks:
            return break_dict
    
    def initiate_encoder(self, break_dict, store_path):
        '''
    ...
    def encode(self, string):   
        ...
        '''
        break_dict = self.fit(string, return_breaks=True)
    ...
    def get_encodings(self, strings):
        enc_dict = {string : self.encode(string) for string in list(set(strings)) if string not in ''}
        
        return [enc_dict[string]  for string in strings]
