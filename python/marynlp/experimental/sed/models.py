...
class SentimentAnalysisModel(Embeddings):
    def __init__(self, token_vocab_size, num_emotions, num_sentiments, rnn_dim, pretrained=False):
        super(SentimentAnalysisModel, self).__init__(token_vocab_size=token_vocab_size, **EmbeddingsConfig.embedder_params)
        
        if not EmbeddingsConfig.embedder_params['batched_input']:
